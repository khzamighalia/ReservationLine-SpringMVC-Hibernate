...
<body style="font-family: 'Montserrat', sans-serif;">
	<nav class="navbar navbar-expand-lg navbar-light bg-light">
		<a class="navbar-brand" href="#"><img
			src="../resources/img/logo.png" style="width: 180px"></img> </a>
				...
				<div class="border" style="width: 360px">
					<div class="form-group ">
						<form action="registeresrv123" method="post">
							<p style="text-align: center;">Ajoutez les horaires
								disponibles !</p>
							<div class="form-group">
								...
								<div class="form-group">
									<label for="exampleInputPassword1">Nombre de places :</label> <input
										type="number" name="nmbr_places" min="1" step="1"
										class="form-control" id="nbrjrs"
										style="background-color: #E8EFF6; border-color: transparent;"
	...
	<div class="container">
		...
		<table class="table ">
			...
			<c:forEach items="${listAcces}" var="p">
				<tr>
					<td>${p.id.user.nom_complet}</td>
					<td>${p.id.gestion.date}</td>
					<td><form action="demandeAcces" method="POST">
							<input type="hidden" name="user_id" value="${p.id.user.id}">
							<input type="hidden" name="reserver_id"
								value="${p.id.gestion.id_gest}">
							<button type=submit style="background-color: white; border: none"
								name="accepter">
								<i class="fas fa-check text-success"></i>
							</button>
	...
	<div class="container">
		<h2 class="text-danger">Historique d'accès</h2>
		<table class="table ">
			<tr>
				<th>Nom Complet</th>
				<th>Date</th>
			...
			<c:forEach items="${listAccess}" var="p">
				<tr>
					<td>${p.id.user.nom_complet}</td>
					<td>${p.id.gestion.date}</td>
					<td><c:choose>
							<c:when test="${ p.etat == 0 }">en attente</c:when>
							<c:when test="${ p.etat == 1 }">approuvée</c:when>
